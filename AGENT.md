...
既存プロダクトやコードベースから invariant を抽出する場合は、次の順で進める。

1. まず code、schema、test、生成物などの source of truth を確認する。
2. 実装の層構造、判断基準、最低限基準を復元する。
3. 各発見事項を `principle` / `capability standard` / `support document` / `project-specific` に分類する。
4. 顧客固有語彙や一時的事情を除き、再利用可能なルールとして書き直す。
5. 実装が部分的な場合は、達成済み capability と将来課題を混同しない。
